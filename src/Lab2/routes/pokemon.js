...
router.route("/history").get(async (req, res) => {
    ...
    try {
        ...
        return res
            .status(500)
            .json({ error: "couldn't parse history", msg: JSON.stringify(e) });
    }
});
...
router.route("/:id").get(async (req, res) => {
    ...
    try {
        ...
        validateRedisListResponse(
            await cache.lPush(`PK_HISTORY`, JSON.stringify(data))
        ); // stringify to use redis lists (which can only hold strings)
