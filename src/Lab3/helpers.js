...
import { authors as authorCollection, books as bookCollection, publishers as publisherCollection } from "./config/mongoCollections.js";
...
export const validateRedisResponse = (res) => {
    if (typeof res !== "string" || res !== "OK") {
        throw new GraphQLError("invalid redis response", { extensions: { code: "INTERNAL_SERVER_ERROR" } });
    }
};
...
export const returnValidId = (id) => {
    ...
    if (typeof id !== "string" || !ObjectId.isValid(id.trim())) 
        throw new GraphQLError(`Invalid id '${id.trim()}'`, { extensions: { code: "BAD_USER_INPUT" } });
...
export const valiDate = (dateOfBirth) => {
    if (typeof dateOfBirth !== "string" || dateOfBirth.trim().length !== 10) // len 10, otherwise invalid format
        throw new GraphQLError("Unexpected date type/format", { extensions: { code: "BAD_USER_INPUT" } });

    dateOfBirth = dateOfBirth.trim();
    const dobArray = dateOfBirth.split("/").map((num) => parseInt(num)); // [MM, DD, YYYY]

    if (!Array.isArray(dobArray) || dobArray.length !== 3) {
        throw new GraphQLError("Unexpected date formatting, parsing error", { extensions: { code: "BAD_USER_INPUT" } });
    }
    ...
    if (
        ...
        throw new GraphQLError(`Date '${dateOfBirth}' is invalid`, { extensions: { code: "BAD_USER_INPUT" } });
    }
    return dateOfBirth;
};
...
export const getBookById = async (bookId) => {
    ...
    if (!book) 
        throw new GraphQLError(`No book with id '${bookId}' found`, { extensions: { code: "NOT_FOUND" } });
    return book;
};
...
export const getAuthorById = async (authorId) => {
    ...
    if (!author) 
        throw new GraphQLError(`No author with id '${authorId}' found`, { extensions: { code: "NOT_FOUND" } });
    return author;
};
...
export const getPublisherById = async (publisherId) => {
    ...
    if (!publisher) 
        throw new GraphQLError(`No publisher with id '${publisherId}' found`, { extensions: { code: "NOT_FOUND" } });
    return publisher;
};
...
export const getChapterById = async (chapterId) => {
    ...
    if (!chapter) 
        throw new GraphQLError(`No chapter with id '${chapterId}' found`, { extensions: { code: "NOT_FOUND" } });
    return chapter;
}
...
export const removeBookFromDatabase = async (bookId, alreadyRemoved) => {
    ...
    if (alreadyRemoved !== "publisher") {
        const updatePub = await publisherCol.updateOne({ _id: book.publisherId }, { $pull: { books: book._id } });
        if (!updatePub.acknowledged || updatePub.matchedCount === 0)
            throw new GraphQLError(`Could not edit publisher with id '${book.publisherId}'`, { extensions: { code: updatePub.matchedCount === 0 ? "NOT_FOUND" : "INTERNAL_SERVER_ERROR" } }); // not found if the update filter didn't match any docs

        const insertedPublisher = await getPublisherById(book.publisherId); // get the publisher
        await updateKeyIfPresent(insertedPublisher, `publisher:${book.publisherId.toString()}`, -1); // update the publisher if in the cache
    ...
    if (alreadyRemoved !== "author") {
        const updateAuth = await authorCol.updateOne({ _id: book.authorId }, { $pull: { books: book._id } });
        if (!updateAuth.acknowledged || updateAuth.matchedCount === 0)
            throw new GraphQLError(`Could not edit author with id '${book.authorId}'`, { extensions: { code: updateAuth.matchedCount === 0 ? "NOT_FOUND" : "INTERNAL_SERVER_ERROR" } }); // not found if the update filter didn't match any docs

        const insertedAuthor = await getAuthorById(book.authorId); // get the author
        await updateKeyIfPresent(insertedAuthor, `author:${book.authorId.toString()}`, -1); // update the author if in the cache
    ...
    if (!deleteInfo || !deleteInfo.acknowledged || deleteInfo.deletedCount !== 1) 
        throw new GraphQLError(`Could not delete book with id '${book._id}'`, { extensions: { code: "INTERNAL_SERVER_ERROR" } });
...
export const addToCache = async (obj, key, expiration) => {
    ...
    if (expiration !== -1) {
        if (!(await cache.expire(key, expiration)))
            throw new GraphQLError("couldn't add expiry time to cached object", { extensions: { code: "INTERNAL_SERVER_ERROR" } });    
    }
};
...
export const updateListIfPresent = async (key) => {
    if (await getFromCache(key)) {
        ...
        if (!result)
            throw new GraphQLError(`Could not fetch ${key}`, { extensions: { code: "INTERNAL_SERVER_ERROR" } });
...
export const updateGenreIfPresent = async (genre) => {
    if (await getFromCache(`genre:${genre}`)) {
        ...
        if (!genreBooks)
            throw new GraphQLError(`Could not fetch books with genre matching '${genre}`, { extensions: { code: "NOT_FOUND" } });
...
export const updateFoundedYearResults = async () => {
    ...
    for (const k of allRanges) {
        const [min, max] = k.split(":").slice(-2).map((str) => parseInt(str)); // get last two elements (min and max)
        const publisherCol = await publisherCollection();
        const allPublishers = await publisherCol.find({ establishedYear: { $gt: min, $lt: max } }).toArray();
        if (!allPublishers)
...
export const updateAuthorSearches = async () => {
    ...
    for (const k of allSearches) {
        ...
        if (!results)
            throw new GraphQLError(`Could not find any authors matching '${searchTerm}'`, { extensions: { code: "NOT_FOUND" } });
...
export const removeFromCacheIfPresent = async (key) => {
    if (await getFromCache(key)) {
        ...
        if (response !== 0) // returns number of paths deleted, should be non-zero
            throw new GraphQLError(`Could not delete ${key}`, { extensions: { code: "INTERNAL_SERVER_ERROR" } });
