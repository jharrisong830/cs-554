...
export default function Page({ type }) { // type in ["events", "attractions", "venues"]
    const API_KEY = "hvGmNotZE5aeMdxx3lO4KApTNzGah58a";
    const { page } = useParams();
    const { data, loading, error } = useAxios(`https://app.ticketmaster.com/discovery/v2/${type}?apikey=${API_KEY}&countryCode=US&page=${parseInt(page) - 1}`);
                    ...
                    <div className="container">
                        ...
                        <div>
                            {parseInt(page) > 1 && // render the below element only if LHS is true, if we are beyond the first page
                                <Link to={`/${type}/page/${parseInt(page) - 1}`} className="button-wrapper">Back</Link>
                            }
                            ...
                            {parseInt(page) < data.page.totalPages && // render only if we are not at the past page
                                <Link to={`/${type}/page/${parseInt(page) + 1}`} className="button-wrapper">Next</Link>
                            }
                        </div>
                    </div>
                )
