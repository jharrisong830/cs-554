...
import { BOOKS_BY_GENRE, PUBLISHERS_BY_ESTABLISHED_YEAR, SEARCH_AUTHOR_BY_NAME, SEARCH_BOOK_BY_TITLE } from "../../resolvers";
    ...
    const getFormControl = () => {
        switch (searchType) {
            ...
            case "publishersByEstablishedYear":
                return (
                    <Form.Group>
                        <Form.Control id="min" type="number" required placeholder="Min year" />
                        <Form.Control id="max" type="number" required placeholder="Max year" />
                    </Form.Group>
                );
            case "searchAuthorByName":
            case "searchBookByTitle":
                return <Form.Control id="searchText" type="text" required placeholder="Search text" />;
        }
    };
    ...
    return (
        <div>
            <Form onSubmit={(e) => handleSearch(e)}>
                <InputGroup>
                    {getFormControl()}
                    <Form.Select id="searchType" defaultValue={searchType} ref={searchTypeSelect} onChange={handleChangeSearchType}>
                        <option value="booksByGenre">Search Books by Genre</option>
                        <option value="publishersByEstablishedYear">Search Publishers by Year</option>
                        <option value="searchAuthorByName">Search Authors by Name</option>
                        <option value="searchBookByTitle">Search Books by Title</option>
                    </Form.Select>
                    <Button type="submit">Search</Button>
                </InputGroup>
            </Form>
            {isShowingResults && <SearchResults searchType={searchType} query={currQuery} queryParams={currParams} setIsShowingResults={setIsShowingResults} />} {/* pass the setter to allow this component to clear the results */}
        </div>
    );
}
