...
import { GET_AUTHOR_BY_ID, EDIT_AUTHOR, GET_AUTHORS_LIST } from "../../../resolvers";
import { toRealDate, toFakeDate } from "../../lib/date";

export default function AuthorModalForm({ authorId, handleClose, setSubmissionStatus }) {
    const { loading, error, data } = useQuery(GET_AUTHOR_BY_ID, {
        variables: { id: authorId },
        fetchPolicy: "cache-and-network"
    });

    const [editAuthor, { loadingMut, errorMut, dataMut }] = useMutation(EDIT_AUTHOR, {
                ...
                error ? <p>Oops! There was an error loading the data. {JSON.stringify(error)}</p> : (
                    <Form onSubmit={(e) => handleSubmit(e)}>
                        <Form.Group>
                            ...
                            <Form.Control id="name" type="text" required defaultValue={data.getAuthorById.name}></Form.Control>
                        </Form.Group>

                        <Form.Group>
                            <Form.Label htmlFor="bio">Biography</Form.Label>
                            <Form.Control id="bio" type="text" required defaultValue={data.getAuthorById.bio}></Form.Control>
                        </Form.Group>

                        <Form.Group>
                            <Form.Label htmlFor="dateOfBirth">Date of Birth</Form.Label>
                            <Form.Control id="dateOfBirth" type="date" required defaultValue={toRealDate(data.getAuthorById.dateOfBirth)}></Form.Control>
                        </Form.Group>
