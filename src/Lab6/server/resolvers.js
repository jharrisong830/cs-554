...
export const resolvers = {
    ...
    Mutation: {
        ...
        editPublisher: async (_, args) => {
            ...
            const insertedPublisher = await getPublisherById(id); // get the publisher
        ...
        addBook: async (_, args) => {
            let {
                ...
                authorId,
                publisherId
            } = args; // extract fields from args (none optional here)
            ...
            for (let i = 0; i < chapters.length; i++) {
                ...
                if (chapters[i].length === 0)
                    throw new GraphQLError("Empty chapter title provided", {
                        extensions: { code: "BAD_USER_INPUT" }
                    });
            }
